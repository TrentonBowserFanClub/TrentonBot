...
#define LATENCY_TIMER                                                          \
  16 // msec (USB latency timer)
     ...
     // When you are going to use sync / bulk read, the latency timer should be
     // loosen. the lower latency timer value, the faster communication speed.
...
bool PortHandlerMac::openPort() { return setBaudRate(baudrate_); }
...
void PortHandlerMac::clearPort() { tcflush(socket_fd_, TCIFLUSH); }

void PortHandlerMac::setPortName(const char *port_name) {
  strcpy(port_name_, port_name);
}

char *PortHandlerMac::getPortName() { return port_name_; }
...
int PortHandlerMac::getBaudRate() { return baudrate_; }
