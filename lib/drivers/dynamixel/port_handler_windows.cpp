...
#define LATENCY_TIMER                                                          \
  16 // msec (USB latency timer)
     // You should adjust the latency timer value. In Windows, the default
     // latency timer of the usb serial is '16 msec'. When you are going to use
     // sync / bulk read, the latency timer should be loosen. the lower latency
     // timer value, the faster communication speed.

// Note:
// You can either checking or changing its value by:
// [Device Manager] -> [Port (COM & LPT)] -> the port you use but starts with
// COMx-> mouse right click -> properties
// -> [port settings] -> [details] -> change response time from 16 to the value
// you need

using namespace dynamixel;

PortHandlerWindows::PortHandlerWindows(const char *port_name)
    : serial_handle_(INVALID_HANDLE_VALUE), baudrate_(DEFAULT_BAUDRATE_),
      packet_start_time_(0.0), packet_timeout_(0.0), tx_time_per_byte_(0.0) {
  is_using_ = false;
...
bool PortHandlerWindows::openPort() { return setBaudRate(baudrate_); }
...
char *PortHandlerWindows::getPortName() { return port_name_; }
...
int PortHandlerWindows::getBaudRate() { return baudrate_; }

int PortHandlerWindows::getBytesAvailable() {
  DWORD retbyte = 2;
  BOOL res = DeviceIoControl(serial_handle_, GENERIC_READ | GENERIC_WRITE, NULL,
                             0, 0, 0, &retbyte, (LPOVERLAPPED)NULL);
...
void PortHandlerWindows::setPacketTimeout(uint16_t packet_length) {
  ...
  packet_timeout_ =
      (tx_time_per_byte_ * (double)packet_length) + (LATENCY_TIMER * 2.0) + 2.0;
}
...
bool PortHandlerWindows::setupPort(int baudrate) {
  ...
  serial_handle_ =
      CreateFileA(port_name_, GENERIC_READ | GENERIC_WRITE, 0, NULL,
  ...
  if (SetupComm(serial_handle_, 4096, 4096) == FALSE) // Buffer size (Rx,Tx)
    goto DXL_HAL_OPEN_ERROR;
  if (PurgeComm(serial_handle_, PURGE_TXABORT | PURGE_TXCLEAR | PURGE_RXABORT |
                                    PURGE_RXCLEAR) == FALSE) // Clear buffer
    goto DXL_HAL_OPEN_ERROR;
