...
#define ERRBIT_VOLTAGE                                                         \
  1 // Supplied voltage is out of the range (operating volatage set in the
    // control table)
#define ERRBIT_ANGLE                                                           \
  2 // Goal position is written out of the range (from CW angle limit to CCW
    // angle limit)
#define ERRBIT_OVERHEAT                                                        \
  4 // Temperature is out of the range (operating temperature set in the control
    // table)
...
#define ERRBIT_OVERLOAD                                                        \
  32 // The current load cannot be controlled by the set torque.
#define ERRBIT_INSTRUCTION                                                     \
  64 // Undefined instruction or delivering the action command without the
     // reg_write command.

using namespace dynamixel;

Protocol1PacketHandler *Protocol1PacketHandler::unique_instance_ =
    new Protocol1PacketHandler();
...
int Protocol1PacketHandler::txPacket(PortHandler *port, uint8_t *txpacket) {
  ...
  uint8_t total_packet_length =
      txpacket[PKT_LENGTH] + 4; // 4: HEADER0 HEADER1 ID LENGTH
  ...
  for (uint16_t idx = 2; idx < total_packet_length - 1;
       idx++) // except header, checksum
  ...
  while (true) {
    ...
    if (rx_length >= wait_length) {
      ...
      if (idx == 0) // found at the beginning of the packet
      {
        ...
        // calculate checksum
        for (uint16_t i = 2; i < wait_length - 1;
             i++) // except header, checksum
          checksum += rxpacket[i];
        checksum = ~checksum;
...
int Protocol1PacketHandler::txRxPacket(PortHandler *port, uint8_t *txpacket,
  ...
  if (txpacket[PKT_INSTRUCTION] == INST_READ) {
    ...
    port->setPacketTimeout(
        (uint16_t)6); // HEADER0 HEADER1 ID LENGTH ERROR CHECKSUM
  }
...
int Protocol1PacketHandler::ping(PortHandler *port, uint8_t id,
                                 uint8_t *error) {
...
int Protocol1PacketHandler::reboot(PortHandler *port, uint8_t id,
                                   uint8_t *error) {
  return COMM_NOT_AVAILABLE;
}

int Protocol1PacketHandler::clearMultiTurn(PortHandler *port, uint8_t id,
                                           uint8_t *error) {
  return COMM_NOT_AVAILABLE;
}

int Protocol1PacketHandler::factoryReset(PortHandler *port, uint8_t id,
...
int Protocol1PacketHandler::readTx(PortHandler *port, uint8_t id,
...
int Protocol1PacketHandler::readRx(PortHandler *port, uint8_t id,
...
int Protocol1PacketHandler::readTxRx(PortHandler *port, uint8_t id,
                                     uint16_t address, uint16_t length,
...
int Protocol1PacketHandler::read1ByteTx(PortHandler *port, uint8_t id,
                                        uint16_t address) {
  return readTx(port, id, address, 1);
}
int Protocol1PacketHandler::read1ByteRx(PortHandler *port, uint8_t id,
                                        uint8_t *data, uint8_t *error) {
...
int Protocol1PacketHandler::read1ByteTxRx(PortHandler *port, uint8_t id,
...
int Protocol1PacketHandler::read2ByteTx(PortHandler *port, uint8_t id,
                                        uint16_t address) {
  return readTx(port, id, address, 2);
}
int Protocol1PacketHandler::read2ByteRx(PortHandler *port, uint8_t id,
...
int Protocol1PacketHandler::read2ByteTxRx(PortHandler *port, uint8_t id,
...
int Protocol1PacketHandler::read4ByteTx(PortHandler *port, uint8_t id,
                                        uint16_t address) {
  return readTx(port, id, address, 4);
}
int Protocol1PacketHandler::read4ByteRx(PortHandler *port, uint8_t id,
...
int Protocol1PacketHandler::read4ByteTxRx(PortHandler *port, uint8_t id,
...
int Protocol1PacketHandler::writeTxOnly(PortHandler *port, uint8_t id,
                                        uint16_t address, uint16_t length,
                                        uint8_t *data) {
  int result = COMM_TX_FAIL;
...
int Protocol1PacketHandler::writeTxRx(PortHandler *port, uint8_t id,
                                      uint16_t address, uint16_t length,
                                      uint8_t *data, uint8_t *error) {
  int result = COMM_TX_FAIL;
...
int Protocol1PacketHandler::write1ByteTxOnly(PortHandler *port, uint8_t id,
                                             uint16_t address, uint8_t data) {
  uint8_t data_write[1] = {data};
  return writeTxOnly(port, id, address, 1, data_write);
}
int Protocol1PacketHandler::write1ByteTxRx(PortHandler *port, uint8_t id,
                                           uint16_t address, uint8_t data,
                                           uint8_t *error) {
  uint8_t data_write[1] = {data};
...
int Protocol1PacketHandler::write2ByteTxOnly(PortHandler *port, uint8_t id,
                                             uint16_t address, uint16_t data) {
  uint8_t data_write[2] = {DXL_LOBYTE(data), DXL_HIBYTE(data)};
  return writeTxOnly(port, id, address, 2, data_write);
}
int Protocol1PacketHandler::write2ByteTxRx(PortHandler *port, uint8_t id,
                                           uint16_t address, uint16_t data,
                                           uint8_t *error) {
  uint8_t data_write[2] = {DXL_LOBYTE(data), DXL_HIBYTE(data)};
...
int Protocol1PacketHandler::write4ByteTxOnly(PortHandler *port, uint8_t id,
                                             uint16_t address, uint32_t data) {
...
int Protocol1PacketHandler::write4ByteTxRx(PortHandler *port, uint8_t id,
                                           uint16_t address, uint32_t data,
                                           uint8_t *error) {
  uint8_t data_write[4] = {
      DXL_LOBYTE(DXL_LOWORD(data)), DXL_HIBYTE(DXL_LOWORD(data)),
...
int Protocol1PacketHandler::regWriteTxOnly(PortHandler *port, uint8_t id,
                                           uint16_t address, uint16_t length,
                                           uint8_t *data) {
  int result = COMM_TX_FAIL;
...
int Protocol1PacketHandler::regWriteTxRx(PortHandler *port, uint8_t id,
                                         uint16_t address, uint16_t length,
                                         uint8_t *data, uint8_t *error) {
  int result = COMM_TX_FAIL;
...
int Protocol1PacketHandler::syncWriteTxOnly(PortHandler *port,
  ...
  txpacket[PKT_LENGTH] =
      param_length + 4; // 4: INST START_ADDR DATA_LEN ... CHKSUM
...
int Protocol1PacketHandler::bulkReadTx(PortHandler *port, uint8_t *param,
                                       uint16_t param_length) {
  int result = COMM_TX_FAIL;
...
int Protocol1PacketHandler::bulkWriteTxOnly(PortHandler *port, uint8_t *param,
                                            uint16_t param_length) {
  return COMM_NOT_AVAILABLE;
}

int Protocol1PacketHandler::fastSyncReadTx(PortHandler *port,
                                           uint16_t start_address,
                                           uint16_t data_length, uint8_t *param,
                                           uint16_t param_length) {
  return COMM_NOT_AVAILABLE;
...
int Protocol1PacketHandler::fastBulkReadTx(PortHandler *port, uint8_t *param,
                                           uint16_t param_length) {
  return COMM_NOT_AVAILABLE;
}