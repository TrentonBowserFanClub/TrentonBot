...
PortHandlerArduino::PortHandlerArduino(const char *port_name)
    : baudrate_(DEFAULT_BAUDRATE_), packet_start_time_(0.0),
...
void PortHandlerArduino::closePort() { setPowerOff(); }
...
double PortHandlerArduino::getCurrentTime() { return (double)millis(); }
