...
#define ERRBIT_ALERT                                                           \
  128 // When the device has a problem, this bit is set to 1. Check "Device
      // Status Check" value.

using namespace dynamixel;

Protocol2PacketHandler *Protocol2PacketHandler::unique_instance_ =
    new Protocol2PacketHandler();
...
const char *Protocol2PacketHandler::getRxPacketError(uint8_t error) {
  if (error & ERRBIT_ALERT)
    return "[RxPacketError] Hardware error occurred. Check the error at "
           "Control Table (Hardware Error Status)!";
  ...
  case ERRNUM_ACCESS:
    return "[RxPacketError] Writing or Reading is not available to target "
           "address!";
...
void Protocol2PacketHandler::addStuffing(uint8_t *packet) {
  int packet_length_in =
      DXL_MAKEWORD(packet[PKT_LENGTH_L], packet[PKT_LENGTH_H]);
  int packet_length_out = packet_length_in;

  if (packet_length_in <
      8) // INSTRUCTION, ADDR_L, ADDR_H, CRC16_L, CRC16_H + FF FF FD
    return;
  ...
  while (out_index != in_index) {
    if (packet[in_index] == 0xFD && packet[in_index - 1] == 0xFF &&
...
int Protocol2PacketHandler::rxPacket(PortHandler *port, uint8_t *rxpacket,
  ...
  uint16_t wait_length = 11; // minimum length (HEADER0 HEADER1 HEADER2 RESERVED
                             // ID LENGTH_L LENGTH_H INST ERROR CRC16_L CRC16_H)
...
int Protocol2PacketHandler::txRxPacket(PortHandler *port, uint8_t *txpacket,
                                       uint8_t *rxpacket, uint8_t *error) {
  int result = COMM_TX_FAIL;
  ...
  if (txpacket[PKT_INSTRUCTION] == INST_READ) {
    port->setPacketTimeout(
        (uint16_t)(DXL_MAKEWORD(txpacket[PKT_PARAMETER0 + 2],
                                txpacket[PKT_PARAMETER0 + 3]) +
                   11));
  } else {
    port->setPacketTimeout((uint16_t)11);
    // HEADER0 HEADER1 HEADER2 RESERVED ID LENGTH_L LENGTH_H INST ERROR CRC16_L
    // CRC16_H
  }
...
int Protocol2PacketHandler::ping(PortHandler *port, uint8_t id,
                                 uint8_t *error) {
...
int Protocol2PacketHandler::ping(PortHandler *port, uint8_t id,
  ...
  if (result == COMM_SUCCESS && model_number != 0)
    *model_number = DXL_MAKEWORD(rxpacket[PKT_PARAMETER0 + 1],
...
int Protocol2PacketHandler::broadcastPing(PortHandler *port,
  ...
  port->setPacketTimeout(((double)wait_length * tx_time_per_byte) +
                         (3.0 * (double)MAX_ID) + 16.0);
  ...
  while (1) {
    ...
    {
      // verify CRC16
      uint16_t crc = DXL_MAKEWORD(rxpacket[STATUS_LENGTH - 2],
...
int Protocol2PacketHandler::reboot(PortHandler *port, uint8_t id,
                                   uint8_t *error) {
  uint8_t txpacket[10] = {0};
...
int Protocol2PacketHandler::clearMultiTurn(PortHandler *port, uint8_t id,
                                           uint8_t *error) {
...
int Protocol2PacketHandler::factoryReset(PortHandler *port, uint8_t id,
...
int Protocol2PacketHandler::readTx(PortHandler *port, uint8_t id,
...
int Protocol2PacketHandler::readRx(PortHandler *port, uint8_t id,
...
int Protocol2PacketHandler::readTxRx(PortHandler *port, uint8_t id,
                                     uint16_t address, uint16_t length,
...
int Protocol2PacketHandler::read1ByteTx(PortHandler *port, uint8_t id,
                                        uint16_t address) {
  return readTx(port, id, address, 1);
}
int Protocol2PacketHandler::read1ByteRx(PortHandler *port, uint8_t id,
                                        uint8_t *data, uint8_t *error) {
...
int Protocol2PacketHandler::read2ByteTx(PortHandler *port, uint8_t id,
                                        uint16_t address) {
  return readTx(port, id, address, 2);
}
int Protocol2PacketHandler::read2ByteRx(PortHandler *port, uint8_t id,
...
int Protocol2PacketHandler::read2ByteTxRx(PortHandler *port, uint8_t id,
...
int Protocol2PacketHandler::read4ByteTx(PortHandler *port, uint8_t id,
                                        uint16_t address) {
  return readTx(port, id, address, 4);
}
int Protocol2PacketHandler::read4ByteRx(PortHandler *port, uint8_t id,
...
int Protocol2PacketHandler::read4ByteTxRx(PortHandler *port, uint8_t id,
...
int Protocol2PacketHandler::writeTxOnly(PortHandler *port, uint8_t id,
                                        uint16_t address, uint16_t length,
                                        uint8_t *data) {
  int result = COMM_TX_FAIL;
...
int Protocol2PacketHandler::writeTxRx(PortHandler *port, uint8_t id,
                                      uint16_t address, uint16_t length,
                                      uint8_t *data, uint8_t *error) {
  int result = COMM_TX_FAIL;
...
int Protocol2PacketHandler::write1ByteTxOnly(PortHandler *port, uint8_t id,
                                             uint16_t address, uint8_t data) {
  uint8_t data_write[1] = {data};
  return writeTxOnly(port, id, address, 1, data_write);
}
int Protocol2PacketHandler::write1ByteTxRx(PortHandler *port, uint8_t id,
                                           uint16_t address, uint8_t data,
                                           uint8_t *error) {
  uint8_t data_write[1] = {data};
...
int Protocol2PacketHandler::write2ByteTxOnly(PortHandler *port, uint8_t id,
                                             uint16_t address, uint16_t data) {
  uint8_t data_write[2] = {DXL_LOBYTE(data), DXL_HIBYTE(data)};
  return writeTxOnly(port, id, address, 2, data_write);
}
int Protocol2PacketHandler::write2ByteTxRx(PortHandler *port, uint8_t id,
                                           uint16_t address, uint16_t data,
                                           uint8_t *error) {
  uint8_t data_write[2] = {DXL_LOBYTE(data), DXL_HIBYTE(data)};
...
int Protocol2PacketHandler::write4ByteTxOnly(PortHandler *port, uint8_t id,
                                             uint16_t address, uint32_t data) {
  uint8_t data_write[4] = {
      DXL_LOBYTE(DXL_LOWORD(data)), DXL_HIBYTE(DXL_LOWORD(data)),
      DXL_LOBYTE(DXL_HIWORD(data)), DXL_HIBYTE(DXL_HIWORD(data))};
  return writeTxOnly(port, id, address, 4, data_write);
}
int Protocol2PacketHandler::write4ByteTxRx(PortHandler *port, uint8_t id,
                                           uint16_t address, uint32_t data,
                                           uint8_t *error) {
  uint8_t data_write[4] = {
      DXL_LOBYTE(DXL_LOWORD(data)), DXL_HIBYTE(DXL_LOWORD(data)),
...
int Protocol2PacketHandler::regWriteTxOnly(PortHandler *port, uint8_t id,
                                           uint16_t address, uint16_t length,
                                           uint8_t *data) {
  int result = COMM_TX_FAIL;
...
int Protocol2PacketHandler::regWriteTxRx(PortHandler *port, uint8_t id,
                                         uint16_t address, uint16_t length,
                                         uint8_t *data, uint8_t *error) {
  int result = COMM_TX_FAIL;
...
int Protocol2PacketHandler::syncReadTx(PortHandler *port,
  ...
  // 14: HEADER0 HEADER1 HEADER2 RESERVED ID LEN_L LEN_H INST START_ADDR_L
  // START_ADDR_H DATA_LEN_L DATA_LEN_H CRC16_L CRC16_H

  if (txpacket == NULL)
    return result;

  txpacket[PKT_ID] = BROADCAST_ID;
  txpacket[PKT_LENGTH_L] =
      DXL_LOBYTE(param_length + 7); // 7: INST START_ADDR_L START_ADDR_H
                                    // DATA_LEN_L DATA_LEN_H CRC16_L CRC16_H
  txpacket[PKT_LENGTH_H] =
      DXL_HIBYTE(param_length + 7); // 7: INST START_ADDR_L START_ADDR_H
                                    // DATA_LEN_L DATA_LEN_H CRC16_L CRC16_H
  txpacket[PKT_INSTRUCTION] = INST_SYNC_READ;
...
int Protocol2PacketHandler::bulkReadTx(PortHandler *port, uint8_t *param,
                                       uint16_t param_length) {
  int result = COMM_TX_FAIL;
  ...
  txpacket[PKT_ID] = BROADCAST_ID;
  txpacket[PKT_LENGTH_L] =
      DXL_LOBYTE(param_length + 3); // 3: INST CRC16_L CRC16_H
  txpacket[PKT_LENGTH_H] =
      DXL_HIBYTE(param_length + 3); // 3: INST CRC16_L CRC16_H
...
int Protocol2PacketHandler::bulkWriteTxOnly(PortHandler *port, uint8_t *param,
                                            uint16_t param_length) {
  int result = COMM_TX_FAIL;
...
int Protocol2PacketHandler::fastSyncReadTx(PortHandler *port,
                                           uint16_t start_address,
                                           uint16_t data_length, uint8_t *param,
                                           uint16_t param_length) {
  int result = COMM_TX_FAIL;
...
int Protocol2PacketHandler::fastBulkReadTx(PortHandler *port, uint8_t *param,
                                           uint16_t param_length) {
  int result = COMM_TX_FAIL;
