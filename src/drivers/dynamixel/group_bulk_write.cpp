...
GroupBulkWrite::GroupBulkWrite(PortHandler *port, PacketHandler *ph)
    : GroupHandler(port, ph), param_length_(0) {
  clearParam();
}
...
bool GroupBulkWrite::addParam(uint8_t id, uint16_t start_address,
...
bool GroupBulkWrite::changeParam(uint8_t id, uint16_t start_address,
                                 uint16_t data_length, uint8_t *data) {
  if (ph_->getProtocolVersion() == 1.0)
    return false;
