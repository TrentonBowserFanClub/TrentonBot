...
GroupBulkRead::GroupBulkRead(PortHandler *port, PacketHandler *ph)
    : GroupHandler(port, ph), last_result_(false) {
  clearParam();
}
...
bool GroupBulkRead::addParam(uint8_t id, uint16_t start_address,
...
bool GroupBulkRead::isAvailable(uint8_t id, uint16_t address,
                                uint16_t data_length) {
  uint16_t start_addr;
  ...
  if (address < start_addr ||
      start_addr + length_list_[id] - data_length < address)
    return false;

  return true;
}

uint32_t GroupBulkRead::getData(uint8_t id, uint16_t address,
                                uint16_t data_length) {
  ...
  case 2:
    return DXL_MAKEWORD(data_list_[id][address - start_addr],
                        data_list_[id][address - start_addr + 1]);

  case 4:
    return DXL_MAKEDWORD(
        DXL_MAKEWORD(data_list_[id][address - start_addr + 0],
                     data_list_[id][address - start_addr + 1]),
        DXL_MAKEWORD(data_list_[id][address - start_addr + 2],
                     data_list_[id][address - start_addr + 3]));
