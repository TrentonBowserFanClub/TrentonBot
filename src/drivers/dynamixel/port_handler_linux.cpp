...
#define LATENCY_TIMER                                                          \
  16 // msec (USB latency timer)
     // You should adjust the latency timer value. From the version
     // Ubuntu 16.04.2, the default latency timer of the usb serial is '16
     // msec'. When you are going to use sync / bulk read, the latency timer
     // should be loosen. the lower latency timer value, the faster
     // communication speed.
...
//
// If you think that the communication is too slow, type following after
// plugging the usb in to change the latency timer
//
// Method 1. Type following (you should do this everytime when the usb once was
// plugged out or the connection was dropped) $ echo 1 | sudo tee
// /sys/bus/usb-serial/devices/ttyUSB0/latency_timer $ cat
// /sys/bus/usb-serial/devices/ttyUSB0/latency_timer
//
// Method 2. If you want to set it as be done automatically, and don't want to
// do above everytime, make rules file in /etc/udev/rules.d/. For example, $
// echo ACTION==\"add\", SUBSYSTEM==\"usb-serial\", DRIVER==\"ftdi_sio\",
// ATTR{latency_timer}=\"1\" > 99-dynamixelsdk-usb.rules $ sudo cp
// ./99-dynamixelsdk-usb.rules /etc/udev/rules.d/ $ sudo udevadm control
// --reload-rules $ sudo udevadm trigger --action=add $ cat
// /sys/bus/usb-serial/devices/ttyUSB0/latency_timer
//
// or if you have another good idea that can be an alternatives,
// please give us advice via github issue
// https://github.com/ROBOTIS-GIT/DynamixelSDK/issues
...
PortHandlerLinux::PortHandlerLinux(const char *port_name)
    : socket_fd_(-1), baudrate_(DEFAULT_BAUDRATE_), packet_start_time_(0.0),
      packet_timeout_(0.0), tx_time_per_byte(0.0) {
  is_using_ = false;
  setPortName(port_name);
}

bool PortHandlerLinux::openPort() { return setBaudRate(baudrate_); }
...
void PortHandlerLinux::clearPort() { tcflush(socket_fd_, TCIFLUSH); }

void PortHandlerLinux::setPortName(const char *port_name) {
  strcpy(port_name_, port_name);
}

char *PortHandlerLinux::getPortName() { return port_name_; }
...
int PortHandlerLinux::getBaudRate() { return baudrate_; }
...
bool PortHandlerLinux::setCustomBaudrate(int speed) {
  ...
  if (closest_speed < speed * 98 / 100 || closest_speed > speed * 102 / 100) {
    printf("[PortHandlerLinux::SetCustomBaudrate] Cannot set speed to %d, "
           "closest is %d \n",
           speed, closest_speed);
    return false;
  }
