...
GroupSyncWrite::GroupSyncWrite(PortHandler *port, PacketHandler *ph,
    ...
    : GroupHandler(port, ph), start_address_(start_address),
...
void GroupSyncWrite::makeParam() {
  ...
  param_ = new uint8_t[id_list_.size() *
                       (1 + data_length_)]; // ID(1) + DATA(data_length)
...
int GroupSyncWrite::txPacket() {
  ...
  return ph_->syncWriteTxOnly(port_, start_address_, data_length_, param_,
